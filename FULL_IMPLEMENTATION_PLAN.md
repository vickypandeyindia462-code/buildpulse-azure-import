...
- 2026-09-15 — Added conversational requirement understanding: the Copilot carries bounded recent context, resolves service references across follow-up turns, asks targeted service/symptom clarifications for vague requests, and retrieves internal evidence only after sufficient intent is available.
...
- 2026-09-15 — Enforced retrieval readiness and context reset: out-of-domain or generic help requests clear stale browser service state, and no repository/Confluence retrieval occurs until the request contains a recognized BuildPulse operational intent.
- 2026-09-15 — Replaced implicit CI selection with slot-gated diagnosis: the Copilot captures intent, then service, then an exact failed job; only that job’s supported diagnosis and fix are sent to Gemini, preventing unrelated service or document output.
